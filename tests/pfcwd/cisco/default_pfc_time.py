...
def find_default_bit_time(interface):
    ...
    if is_pac or is_gb:                                                       # noqa: F821
        ...
        assert speed is not None, \
            "Failed to find matching speed for mac port enum value {}".format(mac_port_speed_enum_val)
...
if __name__ == "__main__":
    bit_time = find_default_bit_time(param_interface)
    print("Setting 512bit register to normal value {}".format(bit_time))
    set_pfc_512bit_time(param_interface, bit_time, 1)
    print(param_success)
